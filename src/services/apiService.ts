...
export const tradingService = {
    ...
    getOrders: async (symbolFilter?: string, dateFilter?: string): Promise<OrderResponse> => {
        try {
            ...
            const response = await api.get(`/trading/orders/list/?${params.toString()}`);
            ...
            const ordersWithTotal = response.data.data.map((order: Order) => ({
                ...order,
                total: order.quantity * Number(order.price)
            }));

            return {
                success: true,
                data: {
                    results: ordersWithTotal
                }
            };
        } catch (error) {
